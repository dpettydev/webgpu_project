...
bool drgMaterial::StartRender(drgRenderContext *context)
{
	...
	// Get our shader data
	drgShaderDataOGL *data = drgMaterialManagerOGL::GetVertexShaderData((int)(*effectNum));
	if (data == NULL)
	{
	...
	drgMaterialDataOGL *platformMaterialData = drgMaterialManagerOGL::GetMaterialData((int)*effectNum);
	...
	{
		glUseProgram(cureffect);
		GL_ERROR("Failed to set the program as active");
	}

	// We need to bind attributes and uniforms here.
	if (context)
	{
		drgMaterialManagerOGL::ApplyUniforms(this, context);
	}
	...
	for (int currentTexture = 0; currentTexture < numSamplers; ++currentTexture)
	{
		...
		glUniform1i(platformMaterialData->m_SamplerLocations[currentTexture], currentTexture);
		GL_ERROR("Failed to set the texture index uniform location");
		...
		if (m_MaterialData.m_Texture[currentTexture] != 0)
		{
			...
			glTextureHandle = (unsigned int)drgTexture::GetTextureGL(texture->m_TexIdx);
			GL_ERROR("Failed to get the OpenGL texture handle");
			glActiveTexture(GL_TEXTURE0 + currentTexture);
			GL_ERROR("Failed making the texture active");
		}
...
#else
		if (drgMaterialManager::m_RenderShadows == PASS_MODE_DIFFUSE)
		{
			((drgTexture *)m_MaterialData.m_Texture[count])->SetAsRenderTexture((drgTexture::DRG_TEX_RENDER_MODE)count);
		}
		else if (count == 0)
