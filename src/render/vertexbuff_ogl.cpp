...
void drgVertBuffer::UnLock(int numVerts)
{
	...
	{
		// assert(0);	// This needs to be implemented. The code below will allow us to write to a dynamic vert buffer without reallocating.
		glBindBuffer(GL_ARRAY_BUFFER, (GLuint)m_VertBufferObject);
		GL_ERROR("Failed to set the dynamic VBO data");
		glBufferSubData(GL_ARRAY_BUFFER, 0, m_NumVerts * m_VertSize, (GLvoid *)m_pVB);
		GL_ERROR("Failed to set the dynamic VBO data");
	}
}
...
{
	glGenBuffers(1, (GLuint*)&m_VertBufferObject);
	GL_ERROR("Failed to generate vertex buffer object");

	glBindBuffer(GL_ARRAY_BUFFER, (GLuint)m_VertBufferObject);
	GL_ERROR("Failed to bind the VBO for setting the data");

	if (!m_bIsDynamic)
	{
		glBufferData(GL_ARRAY_BUFFER, m_NumVerts * m_VertSize, (GLvoid *)m_pVB, GL_STATIC_DRAW);
		GL_ERROR("Failed to set the VBO data");
	}
	else
	{
		glBufferData(GL_ARRAY_BUFFER, m_NumVerts * m_VertSize, (GLvoid *)m_pVB, GL_DYNAMIC_DRAW);
		GL_ERROR("Failed to set the dynamic VBO data");
	}
	if (m_Indexed)
	{
		glGenBuffers(1, (GLuint*)&m_IndexBufferObject);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, (GLuint)m_IndexBufferObject);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_Index.GetNumIndex() * sizeof(unsigned short), (GLvoid *)m_Index.GetIndex(), GL_STATIC_DRAW);
	}
}
...
void drgVertBuffer::ReleaseGLResources()
{
	if (m_VertBufferObject)
	{
		...
		{
			glDeleteBuffers(1, (GLuint*)&m_VertBufferObject);
			GL_ERROR("Failed to delete VBO");
		}
		m_VertBufferObject = 0;
	}
	...
	if (m_IndexBufferObject)
	{
		if (!drgEngine::IsClosing())
		{
			glDeleteBuffers(1, (GLuint*)&m_IndexBufferObject);
			GL_ERROR("Failed to delete IBO");
		}
		m_IndexBufferObject = 0;
	}
}
...
{
	glVertexAttribPointer(_attrib, size, type, (int)normalized, stride, data);
	GL_ERROR("Failed to set the vertex attribute pointer");
	glEnableVertexAttribArray(_attrib);
	GL_ERROR("Failed to enable a vertex attribute array");
}
...
void drgVertBuffer::BindAttributeArrays(unsigned int attributes, void *vertbuf, drgMaterial *material)
{
	...
	drgMaterialDataOGL *materialData = drgMaterialManagerOGL::GetMaterialData(material->GetMaterialNum());
	...
	if (hasWeights)
	{
		drgIntPt offset = (drgIntPt)drgVertHelper::GetVertexWeight0(attributes, vertbuf, 0) - bufferStart;
	...
	if (hasVerts)
	{
		drgIntPt offset = (drgIntPt)drgVertHelper::GetVertexPosition(attributes, vertbuf, 0) - bufferStart;
		SetVertAttribute(materialData->m_PositionAttribute, 3, GL_FLOAT, false, vertSize, (void *)offset);
	}

	if (hasNormals)
	{
		drgIntPt offset = (drgIntPt)drgVertHelper::GetVertexNormal(attributes, vertbuf, 0) - bufferStart;
		SetVertAttribute(materialData->m_NormalAttribute, 3, GL_FLOAT, false, vertSize, (void *)offset);
	}

	if (hasTex)
	{
		drgIntPt offset = (drgIntPt)drgVertHelper::GetVertexTexCoord0(attributes, vertbuf, 0) - bufferStart;
	...
	{
		drgIntPt offset = (drgIntPt)drgVertHelper::GetVertexColor0(attributes, vertbuf, 0) - bufferStart;
...
void drgVertBuffer::Render(unsigned int start, unsigned int numPrims, bool useEffect, bool setup, drgMaterial *material)
{
	...
	if (setup)
	{
		...
		if (useEffect)
		{
			...
			if (renderMaterial && setup && useEffect)
				renderMaterial->StartRender(m_Context);
		}
		...
		if (!m_bIsDynamic)
		{
			//if (m_CurrentVertObject != m_VertObject)
			{
				...
				//glBindVertexArray(m_VertObject); GL_ERROR();

				glBindBuffer(GL_ARRAY_BUFFER, (GLuint)m_VertBufferObject);
				GL_ERROR("Failed to bind the vertex buffer object");
				if (m_Indexed)
				{
					glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, (GLuint)m_IndexBufferObject);
					GL_ERROR("Failed to bind the index buffer object");
				}
	...
	assert(m_Context);
	m_Context->RenderPrimitive(m_RenderFlags, m_PrimitiveType, numPrims, m_NumVerts, m_pVB, m_VertSize, indeces, useEffect, start);
