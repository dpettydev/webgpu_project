...
#define	JPGALLOC(x)			malloc(x)
#define	JPGREALLOC(x, y)	realloc(x, y)
#define	JPGFREE(x)			free(x)
...
void *jpeg_decoder::alloc(int n)
{
  ...
  return ((void *)(((unsigned long long)q + 7) & ~7));
}
//------------------------------------------------------------------------------
...
void jpeg_decoder::prep_in_buffer(void)
{
  ...
  do
  {
    int bytes_read = Pstream->readjpg(in_buf + in_buf_left, JPGD_INBUFSIZE - in_buf_left, &eof_flag);
    if (bytes_read == -1)
...
void jpeg_decoder::process_restart(void)
{
  ...
  memset(&last_dc_val, 0, comps_in_frame * sizeof(unsigned int));
...
void jpeg_decoder::init_frame(void)
{
  ...
  q = (unsigned char *)(((unsigned long long)q + 7) & ~7);
  ...
  Psample_buf = (unsigned char *)(((unsigned long long)alloc(max_blocks_per_row * 64 + 8) + 7) & ~7);
...
void jpeg_decoder::start(Pjpeg_decoder_stream Pstream)
{
  if (setjmp(jmp_state))
    return;
